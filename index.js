...
function getWinWord(date) {
  ...
  const dateUTC = new Date(
    Date.UTC(date.getYear() + 1900, date.getMonth(), date.getDate())
  );
...
function init() {
  ...
  document.getElementById("copyGame").addEventListener("click", (e) => {
    ...
    const gameColors = guesses.map((guess) =>
      getColours(guess, winWordAscii).join("")
    );
    const score =
  ...
  document
    ...
    .addEventListener("click", onHistoryButtonClicked);
...
function addWord(word, currIdx, slowly) {
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (colours[i] === GREEN) {
      ...
      changeKeyColors.push(() =>
        getKeyboardKey(word[i]).classList.add("good", "position")
      );
  ...
  if (slowly) {
    cells[currIdx - 1].addEventListener("transitionend", () =>
...
function getKeyboardKey(letter) {
  return Array.from(document.querySelectorAll(".key > div")).find(
    (e) => e.innerHTML === letter
  );
}
...
function onHistoryButtonClicked() {
  ...
  const scoreAvg =
    scores.length == 0
      ? ""
      : Number(scores.reduce((x, y) => x + y) / scores.length).toFixed(1);
